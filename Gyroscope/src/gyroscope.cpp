...
void TextBoardWidget::setValue(int index, double value)
{
    if (0 == index) {
        m_nPitchValue = value;
    } else if (1 == index ) {
...
void TextBoardWidget::paintEvent(QPaintEvent *)
{
    ...
    QString strTemp = QString("%1 : %2 %3").arg(m_strLabels.at(0)).arg(QString::number(m_nPitchValue, 'f', 3)).arg(m_strUnits.at(0));
    painter.drawText(rect, strTemp);

    rect = QRect(rect.left(), rect.bottom() + 30, rect.width(), rect.height());
    //strTemp = QString("%1 : %2 %3").arg(m_strLabels.at(1)).arg(m_nRollValue, 4, 10).arg(m_strUnits.at(1));
    strTemp = QString("%1 : %2 %3").arg(m_strLabels.at(1)).arg(QString::number(m_nRollValue, 'f', 3)).arg(m_strUnits.at(1));
    painter.drawText(rect, strTemp);

    rect = QRect(rect.left(), rect.bottom() + 30, rect.width(), rect.height());
    //strTemp = QString("%1 : %2 %3").arg(m_strLabels.at(2)).arg(m_nYawValue, 4, 10).arg(m_strUnits.at(2));
    strTemp = QString("%1 : %2 %3").arg(m_strLabels.at(2)).arg(QString::number(m_nYawValue, 'f', 3)).arg(m_strUnits.at(2));
    painter.drawText(rect, strTemp);
}
...
Gyroscope::Gyroscope(QWidget *parent) : QtAnimationWidget(parent)
{
    ...
    connect(m_threadTest, SIGNAL(signalUpdate(int,int,double)), this, SLOT(SltUpdateValues(int,int,double)));
    m_threadTest->start();
#else
...
{
//    QFile in_accel_x_raw(this->mpuDevice+"/in_accel_x_raw");
...
//    QFile in_accel_y_raw(this->mpuDevice+"/in_accel_y_raw");
...
//    QFile in_accel_z_raw(this->mpuDevice+"/in_accel_z_raw");
...
//    QFile in_anglvel_x_raw(this->mpuDevice+"/in_anglvel_x_raw");
...
//    QFile in_anglvel_y_raw(this->mpuDevice+"/in_anglvel_y_raw");
...
//    QFile in_anglvel_z_raw(this->mpuDevice+"/in_anglvel_z_raw");
...
void Gyroscope::SltUpdateValues(int type, int index, double value)
{
    if (0 == type) {
        m_textBoardLeft->setValue(index, value);
    } else {
