/******************************************************************
 ...
 文 件 名 : qtcalendarwidget.cpp --- QtCalendarWidget
 作 者    : Niyh(lynnhua)
...
LunarCalendarInfo::~LunarCalendarInfo()
{
    lunarCalendarTable.clear();
    springFestival.clear();
    lunarData.clear();
    chineseTwentyFourData.clear();
    monthAdd.clear();
    listDayName.clear();
    listMonthName.clear();
    listSolarTerm.clear();
...
bool LunarCalendarInfo::isToday(const QDate &date)
{
    QDate cd = QDate::currentDate();
    return ((cd.year() == date.year()) && (cd.month() == date.month()) && (cd.day() == date.day()));
}
...
QString LunarCalendarInfo::getWeekName(int week, int type)
{
    QString strWeek = "";
    if (0 == type) {
        switch (week) {
...
QDate LunarCalendarInfo::getNextMonth(const QDate &date)
{
    int nMonth = date.month() + 1;
    int nYear  = date.year();
    ...
    return QDate(nYear, nMonth, date.day());
}

QDate LunarCalendarInfo::getPrevMonth(const QDate &date)
{
    int nMonth = date.month() - 1;
    int nYear  = date.year();
...
QtCalendarWidget::QtCalendarWidget(QWidget *parent) : QtWidgetBase(parent)
{
    ...
    m_nCurrentYear = m_currentDate.year();
    m_nCurrentMonth = m_currentDate.month();
    m_nCurrendDay = m_currentDate.day();
...
{
    if (NULL != m_lunarCalendarInfo) {
        delete m_lunarCalendarInfo;
        m_lunarCalendarInfo = NULL;
    }
}

void QtCalendarWidget::SetFont(QFont font)
{
    m_font = font;
    this->update();
}

void QtCalendarWidget::SetBackground(const QColor &color)
{
    if (color.isValid()) {
...
void QtCalendarWidget::SetCellSize(int w, int h)
{
    m_cellWidth = w;
    m_cellHeight = h;
    CalcRectSize();
...
void QtCalendarWidget::setXPos(int nValue)
{
    m_nStartPos = nValue;
    if (m_nStartPos == m_nMoveEndValue) {
        if (!m_bRecovery && LeftDirection == m_nDirection)  {
            m_currentDate = m_lunarCalendarInfo->getNextMonth(m_currentDate);
            emit currentDateChanged(m_currentDate);
        }
        else if (!m_bRecovery && RightDirection == m_nDirection) {
            m_currentDate = m_lunarCalendarInfo->getPrevMonth(m_currentDate);
            emit currentDateChanged(m_currentDate);
        }
...
void QtCalendarWidget::paintEvent(QPaintEvent *)
{
    ...
    else if (LeftDirection == m_nDirection) {
        drawDays(&painter, m_currentDate, m_nStartPos);
        if (m_nStartPos < m_calendarRect.left()) {
            drawDays(&painter, m_lunarCalendarInfo->getNextMonth(m_currentDate),
                     m_calendarRect.right() + m_nStartPos);
        }
        else {
            drawDays(&painter, m_lunarCalendarInfo->getPrevMonth(m_currentDate),
                     -m_calendarRect.width() + m_nStartPos);
        }
    }
    else if (RightDirection == m_nDirection) {
        drawDays(&painter, m_currentDate, m_nStartPos);
        if ((m_nStartPos + m_calendarRect.width()) < m_calendarRect.right()) {
            drawDays(&painter, m_lunarCalendarInfo->getNextMonth(m_currentDate),
                     m_calendarRect.right() + m_nStartPos);
        }
        else {
            drawDays(&painter, m_lunarCalendarInfo->getPrevMonth(m_currentDate),
                     m_calendarRect.left() + m_nStartPos - m_calendarRect.width());
        }
    }
}

void QtCalendarWidget::drawTitle(QPainter *painter)
{
    painter->save();
    ...
    for (int i = 0; i < 7; i++) {
        QRect rect(m_calendarRect.left() + i * m_cellWidth, m_calendarRect.top() - TITLE_HEIGHT, m_cellWidth, TITLE_HEIGHT);
        painter->drawText(rect, Qt::AlignCenter, m_lunarCalendarInfo->getWeekName(i));
    }
    painter->restore();
}

void QtCalendarWidget::drawDays(QPainter *painter, const QDate &date, int nStartPos)
{
    painter->save();
    QFont font = painter->font();
    font.setPixelSize(20);
    painter->setFont(font);
    painter->setPen(m_colorText);
    painter->setClipRect(m_calendarRect);

    int week = m_lunarCalendarInfo->getFirstDayOfWeek(date.year(), date.month());
    // 当前月天数
    ...
    for (int i = 0; i < 42; i++) {
        if ((i < week) || (i >= (week + countDay))) continue;
        nIndex = i - week + 1;
        QRect rect = QRect((i % 7) * m_cellWidth + m_calendarRect.left() + nStartPos,
                           (i / 7) * m_cellHeight + m_calendarRect.top(),
                           m_cellWidth, m_cellHeight);

        QString strLunar = m_lunarCalendarInfo->getLunarDay(date.year(), date.month(), nIndex);
        QRect rectNumber(rect.left(), rect.top(), rect.width(), rect.height() * 0.66);
        QRect rectLunar(rect.left(), rect.bottom() - rect.height() / 2, rect.width(), rect.height() * 1.0 / 3);

        if (m_lunarCalendarInfo->isToday(QDate(date.year(), date.month(), nIndex))) {
            painter->save();
            painter->setPen("#333333");
            painter->setBrush(QColor("#ffffff"));
            painter->drawEllipse(rect.left() + (rect.width() - nEllipseW) / 2,
                                 rect.top() + (rect.height() - nEllipseW) / 2,
                                 nEllipseW, nEllipseW);
            painter->drawText(rectNumber, Qt::AlignCenter, QString::number(nIndex));

            font.setPixelSize(12);
            painter->setFont(font);
            painter->setPen("#666666");
            painter->drawText(rectLunar, Qt::AlignCenter, strLunar);
            painter->restore();
...
void QtCalendarWidget::mousePressEvent(QMouseEvent *e)
{
    m_bPressed = true;
    m_startPos = e->pos();
...
void QtCalendarWidget::mouseReleaseEvent(QMouseEvent *e)
{
    m_bPressed = false;
