    ...
    QtAnimationWidget(parent)
{
    ...
    mnesThread = new mNesThread(this);
    mnesThread->stop();
    applicationQWidget = this;


    neswidget = new NesScreenWidget(NULL);
    connect(mnesThread, SIGNAL(loadFrame()), neswidget, SLOT(loadFrame()));

}
void NesWidget::setFileName(QString fileName)
{
    qDebug()<<fileName;

    if (fileName.isEmpty() || (m_strFileName == fileName))
        return;

    if (mnesThread->isRunning())
        mnesThread->stop();

    if(0 == InfoNES_Load(fileName.toLocal8Bit().data()))
    {
        nesThreadRun = true;
 ...
 */
void NesWidget::keyPressEvent(QKeyEvent *event)
{
    switch(event->key())
    {
...
void NesWidget::keyReleaseEvent(QKeyEvent *event)
{
    switch(event->key())
    {
    case Qt::Key_Space://space : A
...
void NesWidget::closeEvent(QCloseEvent *event)
{
...
//    emit newWindow->nesThread->loadFrame();
    NesWidget *newWindow = (NesWidget *) applicationQWidget;
    emit newWindow->mnesThread->loadFrame();
}
...
//////////////////////////////////////////////////////////////////
mNesThread::mNesThread(QObject *parent) : QThread(parent)
{
    m_nRunFlag = 0;
}

void mNesThread::stop()
{
    m_nRunFlag = 0;
    this->quit();
    this->wait();
